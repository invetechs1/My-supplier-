...
export interface Summary { min_price?: number | null; avg_price?: number | null; max_price?: number | null; offer_count: number; supplier_count: number; change_30d_pct?: number | null }
...
export interface Order { id: number; total: number; status: string; created_at: string; supplier?: { name: string } | null; buyer_name: string; items: { description: string; quantity: number; unit: string; unit_price: number }[] }
