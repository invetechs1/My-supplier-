...
export function layout(title: string, body: string, cta?: { label: string; url: string }) {
  ...
  <div style="max-width:560px;margin:24px auto;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #e5e7eb">
    ...
    <div style="padding:24px">
      ...
      ${cta ? `<p style="margin:24px 0 8px"><a href="${cta.url}" style="background:#F2A900;color:#111827;text-decoration:none;font-weight:700;padding:12px 20px;border-radius:10px;display:inline-block">${cta.label}</a></p>` : ""}
      <p style="color:#6b7280;font-size:12px;margin-top:24px">MySupplier · Riyadh, Saudi Arabia · Prices in SAR incl. VAT where stated.</p>
    </div></div></body></html>`;
}
