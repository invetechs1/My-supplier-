...
export function CookieBanner() {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 px-4 pb-4 sm:px-6" role="dialog" aria-live="polite" aria-label={t("cookie.label")} style={{ paddingBottom: "max(1rem, env(safe-area-inset-bottom))" }}>
      <div className="mx-auto flex max-w-3xl flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-xl sm:flex-row sm:items-center">
        <p className="flex-1 text-sm text-slate-600">
          {t("cookie.text")}{" "}
          <Link href="/privacy" className="font-semibold text-brand-700 hover:underline">{t("footer.privacy")}</Link>.
        </p>
        <div className="flex shrink-0 gap-2">
          <Button variant="outline" size="sm" onClick={() => choose("essential")}>{t("cookie.essential")}</Button>
          <Button size="sm" onClick={() => choose("accepted")}>{t("cookie.accept")}</Button>
        </div>
      </div>
    </div>
  );
