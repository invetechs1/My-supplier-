      ...
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-xs text-slate-500">
            {q.user?.name ?? "Buyer"} · {formatDate(q.createdAt, lang)}
          </p>
        </div>
      </div>
      {q.answer ? (
        <div className="mt-3 flex gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 text-xs text-slate-500">
              {q.answeredBy?.companyName ?? q.answeredBy?.name ?? "Supplier"}
        ...
        <p className="ms-9 mt-2 text-xs text-slate-400">
          <Badge tone="amber">Awaiting answer</Badge>
        </p>
      )}
    </li>
  );
...
export function ProductQuestions({ productId, initialCount }: { productId: string; initialCount: number }) {
  ...
  const ask = async () => {
    ...
    try {
      ...
      setError(errorMessage(err, "Could not post your question."));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Card id="questions" className="scroll-mt-24">
      <CardHeader title={`Questions & answers${total ? ` (${total})` : ""}`} subtitle="Suppliers who sell this product are notified and answer here." />
      <div className="grid gap-6 px-5 py-5 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="min-w-0">
          {questions.loading && !questions.data ? (
            ...
            <EmptyState title="No questions yet" description="Ask about specifications, certificates, delivery or packaging." />
          ) : (
            <>
        ...
        <aside className="rounded-xl border border-slate-200 bg-slate-50 p-4">
          <h4 className="text-sm font-semibold text-slate-900">Have a question?</h4>
          {user ? (
            <form
            ...
            >
              <Textarea name="question" value={text} onChange={(e) => setText(e.target.value)} maxLength={1000} placeholder="e.g. Is this available with a SASO certificate?" aria-label="Your question" />
              {error && <Alert>{error}</Alert>}
              {sent && !error && <Alert kind="success">Question posted — we will notify you when it is answered.</Alert>}
