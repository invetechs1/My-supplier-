  ...
  return (
    <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col justify-center px-4 py-12">
      ...
      <Card className="p-6">
        ...
        <Link href={`/login?method=phone${safeNext(params.get("next")) ? `&next=${encodeURIComponent(safeNext(params.get("next")) as string)}` : ""}`} className="flex h-11 w-full items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white text-sm font-semibold text-slate-700 hover:bg-slate-50">
