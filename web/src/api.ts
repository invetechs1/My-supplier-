...
export interface Supplier extends SupplierBrief { user_id: number | null; cr_number: string; vat_number: string; regions: string[]; category_ids: number[]; description: string; phone: string; plan: string; created_at: string; offer_count: number; iban_masked: string; bank_name: string }
...
export interface Order { id: number; buyer_id: number; supplier_id: number; rfq_id: number | null; bid_id: number | null; subtotal: number; vat: number; delivery_fee: number; total: number; currency: string; status: string; payment_status: string; delivery_address: string; city: string; notes: string; created_at: string; updated_at: string; items: OrderItem[]; supplier?: SupplierBrief | null; buyer_name: string; has_review: boolean }
