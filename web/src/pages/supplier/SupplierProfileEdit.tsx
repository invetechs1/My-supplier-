...
export default function SupplierProfileEdit() {
  ...
  useEffect(() => { if (res.data) setF({ name: res.data.name, cr_number: res.data.cr_number, vat_number: res.data.vat_number, city: res.data.city, description: res.data.description, website: res.data.website, phone: res.data.phone, delivery_available: res.data.delivery_available, lead_time_days: res.data.lead_time_days, category_ids: res.data.category_ids, regions: res.data.regions, iban: '', bank_name: res.data.bank_name, delivery_fee: res.data.delivery_fee ?? 0, free_delivery_over: res.data.free_delivery_over ?? '', min_order_amount: res.data.min_order_amount ?? 0 }) }, [res.data])
  if (!f) return <Spinner />
  const save = async (e: FormEvent) => { e.preventDefault(); try { const body = { ...f }; if (!body.iban) delete body.iban; body.free_delivery_over = body.free_delivery_over === '' ? null : Number(body.free_delivery_over); body.delivery_fee = Number(body.delivery_fee || 0); body.min_order_amount = Number(body.min_order_amount || 0); await api.put('/suppliers/me', body); setMsg({ kind: 'ok', text: t('success') }) } catch (ex: any) { setMsg({ kind: 'error', text: ex.message }) } }
  return (
    <form className="stack" onSubmit={save}>
      ...
      <div className="card"><h3>🚚 {t('shipping_settings')}</h3><div className="grid grid-3">
        <div className="field"><label>{t('delivery_fee_label')}</label><input type="number" min={0} step="0.01" value={f.delivery_fee} onChange={e => setF({ ...f, delivery_fee: e.target.value })} /></div>
        <div className="field"><label>{t('free_delivery_over')}</label><input type="number" min={0} step="0.01" value={f.free_delivery_over} onChange={e => setF({ ...f, free_delivery_over: e.target.value })} placeholder="—" /></div>
        <div className="field"><label>{t('min_order_amount')}</label><input type="number" min={0} step="0.01" value={f.min_order_amount} onChange={e => setF({ ...f, min_order_amount: e.target.value })} /></div>
      </div></div>
