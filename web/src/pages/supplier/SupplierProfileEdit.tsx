...
export default function SupplierProfileEdit() {
  ...
  useEffect(() => { if (res.data) setF({ name: res.data.name, cr_number: res.data.cr_number, vat_number: res.data.vat_number, city: res.data.city, description: res.data.description, website: res.data.website, phone: res.data.phone, delivery_available: res.data.delivery_available, lead_time_days: res.data.lead_time_days, category_ids: res.data.category_ids, regions: res.data.regions }) }, [res.data])
  if (!f) return <Spinner />
  const save = async (e: FormEvent) => { e.preventDefault(); try { await api.put('/suppliers/me', f); setMsg({ kind: 'ok', text: t('success') }) } catch (ex: any) { setMsg({ kind: 'error', text: ex.message }) } }
  return (
